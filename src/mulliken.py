...
def overlap(X1,Y1,Z1,X2,Y2,Z2,mu1,mu2,type1,type2):
    ...
    pval=0.5*(mu1+mu2)*r
    tval=(mu1-mu2)/(mu1+mu2)
    res=0
    vector=[[1,0,0],[0,1,0],[0,0,1]]
    # s-s
    if type1==1 and type2==1:
        res=Smulliken_s_s(pval,tval,type1,type2)
    ...
    if type1==2 and type2==1:
        res=Smulliken_s_s(pval,-tval,type2,type1)
    ...
    if type1==1 and (type2>=3 and type2<=5):
        ...
        S=Smulliken_s_psigma(pval,tval,type1,type2)
    ...
    if type2==1 and (type1>=3 and type1<=5):
        ...
        S=Smulliken_s_psigma(pval,-tval,type2,type1)
    ...
    if (type1>=3 and type1<=5) and (type2>=3 and type2<=5):
        ...
        S=Smulliken_psigma_psigma(pval,tval,type1,type2)
        Spi=Smulliken_ppi_ppi(pval,tval,type1,type2)
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
    ...
    if (type2>=3 and type2<=5) and (type1>=7 and type1<=9):
        ...
        S=Smulliken_psigma_psigma(pval,-tval,type2,type1)
        Spi=Smulliken_ppi_ppi(pval,-tval,type2,type1)
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
...
def intval(xval,rmin,dr,coeffs):
    j=int((xval-rmin)/dr)
    return coeffs[j][0]+coeffs[j][1]*xval+coeffs[j][2]*xval**2+coeffs[j][3]*xval**3

def overlap_splines(X1,Y1,Z1,X2,Y2,Z2,mu1,mu2,type1,type2,mu_dict,s_s_mapping,s_psigma_mapping,psigma_psigma_mapping,ppi_ppi_mapping,rmin,rmax,dr):    
    r=math.sqrt((X1-X2)**2+(Y1-Y2)**2+(Z1-Z2)**2)
    pval=0.5*(mu1+mu2)*r
    tval=(mu1-mu2)/(mu1+mu2)
    res=0
    ...
    if type1==1 and type2==1:        
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}1{mu_dict[f"{mu2:.6f}"]}1']['coeffs'])        
    if type1==2 and type2==2:        
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}2{mu_dict[f"{mu2:.6f}"]}2']['coeffs'])        
    if type1==6 and type2==6:        
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}6{mu_dict[f"{mu2:.6f}"]}6']['coeffs'])        
    if type1==1 and type2==2:        
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}1{mu_dict[f"{mu2:.6f}"]}2']['coeffs'])        
    if type1==2 and type2==1:        
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}2{mu_dict[f"{mu2:.6f}"]}1']['coeffs'])        
    if type1==1 and type2==6:
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}1{mu_dict[f"{mu2:.6f}"]}6']['coeffs'])        
    if type1==6 and type2==1:
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}6{mu_dict[f"{mu2:.6f}"]}1']['coeffs'])            
    if type1==2 and type2==6:
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}2{mu_dict[f"{mu2:.6f}"]}6']['coeffs'])        
    if type1==6 and type2==2:        
        res=0
        if r<rmax:
            res=intval(r,rmin,dr,s_s_mapping[f'{mu_dict[f"{mu1:.6f}"]}6{mu_dict[f"{mu2:.6f}"]}2']['coeffs'])        
    
    # s-p
    if type1==1 and (type2>=3 and type2<=5):        
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}1{mu_dict[f"{mu2:.6f}"]}3']['coeffs'])        
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        res=vector[type2-2-1][0]*ezb[0]+vector[type2-2-1][1]*ezb[1]+vector[type2-2-1][2]*ezb[2]
        res=res*S
    ...
    if type2==1 and (type1>=3 and type1<=5):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}3{mu_dict[f"{mu2:.6f}"]}1']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
        res=vector[type1-2-1][0]*ezb[0]+vector[type1-2-1][1]*ezb[1]+vector[type1-2-1][2]*ezb[2]
        res=res*S 
    if type1==2 and (type2>=3 and type2<=5):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}2{mu_dict[f"{mu2:.6f}"]}3']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        res=vector[type2-2-1][0]*ezb[0]+vector[type2-2-1][1]*ezb[1]+vector[type2-2-1][2]*ezb[2]
        res=res*S 
    if type2==2 and (type1>=3 and type1<=5):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}3{mu_dict[f"{mu2:.6f}"]}2']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
        res=vector[type1-2-1][0]*ezb[0]+vector[type1-2-1][1]*ezb[1]+vector[type1-2-1][2]*ezb[2]
        res=res*S
    if type1==1 and (type2>=7 and type2<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}1{mu_dict[f"{mu2:.6f}"]}7']['coeffs'])  
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        res=vector[type2-6-1][0]*ezb[0]+vector[type2-6-1][1]*ezb[1]+vector[type2-6-1][2]*ezb[2]
        res=res*S
    if type2==1 and (type1>=7 and type1<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}7{mu_dict[f"{mu2:.6f}"]}1']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
        res=vector[type1-6-1][0]*ezb[0]+vector[type1-6-1][1]*ezb[1]+vector[type1-6-1][2]*ezb[2]
        res=res*S
    if type1==2 and (type2>=7 and type2<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}2{mu_dict[f"{mu2:.6f}"]}7']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        res=vector[type2-6-1][0]*ezb[0]+vector[type2-6-1][1]*ezb[1]+vector[type2-6-1][2]*ezb[2]
        res=res*S
    if type2==2 and (type1>=7 and type1<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}7{mu_dict[f"{mu2:.6f}"]}2']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
        res=vector[type1-6-1][0]*ezb[0]+vector[type1-6-1][1]*ezb[1]+vector[type1-6-1][2]*ezb[2]
        res=res*S
    if (type1>=3 and type1<=5) and type2==6:
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}3{mu_dict[f"{mu2:.6f}"]}6']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        res=vector[type1-2-1][0]*ezb[0]+vector[type1-2-1][1]*ezb[1]+vector[type1-2-1][2]*ezb[2]
        res=res*S
    if (type2>=3 and type2<=5) and type1==6:
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}6{mu_dict[f"{mu2:.6f}"]}3']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
        res=vector[type2-2-1][0]*ezb[0]+vector[type2-2-1][1]*ezb[1]+vector[type2-2-1][2]*ezb[2]
        res=res*S
    if type1==6 and (type2>=7 and type2<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}6{mu_dict[f"{mu2:.6f}"]}7']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        res=vector[type2-6-1][0]*ezb[0]+vector[type2-6-1][1]*ezb[1]+vector[type2-6-1][2]*ezb[2]
        res=res*S
    if type2==6 and (type1>=7 and type1<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,s_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}7{mu_dict[f"{mu2:.6f}"]}6']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
        res=vector[type1-6-1][0]*ezb[0]+vector[type1-6-1][1]*ezb[1]+vector[type1-6-1][2]*ezb[2]
        res=res*S
    ...
    if (type1>=3 and type1<=5) and (type2>=3 and type2<=5):        
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,psigma_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}3{mu_dict[f"{mu2:.6f}"]}3']['coeffs'])
        Spi=0
        if r<rmax:
            Spi=intval(r,rmin,dr,ppi_ppi_mapping[f'{mu_dict[f"{mu1:.6f}"]}3{mu_dict[f"{mu2:.6f}"]}3']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        r1=(vector[type1-2-1][0]*ezb[0]+vector[type1-2-1][1]*ezb[1]+vector[type1-2-1][2]*ezb[2])
        r1=r1*(vector[type2-2-1][0]*ezb[0]+vector[type2-2-1][1]*ezb[1]+vector[type2-2-1][2]*ezb[2])
        r2=(vector[type1-2-1][0]*eyb[0]+vector[type1-2-1][1]*eyb[1]+vector[type1-2-1][2]*eyb[2])
    ...
    if (type1>=3 and type1<=5) and (type2>=7 and type2<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,psigma_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}3{mu_dict[f"{mu2:.6f}"]}7']['coeffs'])
        Spi=0
        if r<rmax:
            Spi=intval(r,rmin,dr,ppi_ppi_mapping[f'{mu_dict[f"{mu1:.6f}"]}3{mu_dict[f"{mu2:.6f}"]}7']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        r1=   (vector[type1-2-1][0]*ezb[0]+vector[type1-2-1][1]*ezb[1]+vector[type1-2-1][2]*ezb[2])
        r1=r1*(vector[type2-6-1][0]*ezb[0]+vector[type2-6-1][1]*ezb[1]+vector[type2-6-1][2]*ezb[2])
        r2=   (vector[type1-2-1][0]*eyb[0]+vector[type1-2-1][1]*eyb[1]+vector[type1-2-1][2]*eyb[2])
    ...
    if (type2>=3 and type2<=5) and (type1>=7 and type1<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,psigma_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}7{mu_dict[f"{mu2:.6f}"]}3']['coeffs'])
        Spi=0
        if r<rmax:
            Spi=intval(r,rmin,dr,ppi_ppi_mapping[f'{mu_dict[f"{mu1:.6f}"]}7{mu_dict[f"{mu2:.6f}"]}3']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X2,Y2,Z2,X1,Y1,Z1)
        r1=   (vector[type2-2-1][0]*ezb[0]+vector[type2-2-1][1]*ezb[1]+vector[type2-2-1][2]*ezb[2])
        r1=r1*(vector[type1-6-1][0]*ezb[0]+vector[type1-6-1][1]*ezb[1]+vector[type1-6-1][2]*ezb[2])
        r2=   (vector[type2-2-1][0]*eyb[0]+vector[type2-2-1][1]*eyb[1]+vector[type2-2-1][2]*eyb[2])
    ...
    if (type1>=7 and type1<=9) and (type2>=7 and type2<=9):
        S=0
        if r<rmax:
            S=intval(r,rmin,dr,psigma_psigma_mapping[f'{mu_dict[f"{mu1:.6f}"]}7{mu_dict[f"{mu2:.6f}"]}7']['coeffs'])
        Spi=0
        if r<rmax:
            Spi=intval(r,rmin,dr,ppi_ppi_mapping[f'{mu_dict[f"{mu1:.6f}"]}7{mu_dict[f"{mu2:.6f}"]}7']['coeffs'])
        exb,eyb,ezb=MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2)
        r1=   (vector[type1-6-1][0]*ezb[0]+vector[type1-6-1][1]*ezb[1]+vector[type1-6-1][2]*ezb[2])
        r1=r1*(vector[type2-6-1][0]*ezb[0]+vector[type2-6-1][1]*ezb[1]+vector[type2-6-1][2]*ezb[2])
        r2=   (vector[type1-6-1][0]*eyb[0]+vector[type1-6-1][1]*eyb[1]+vector[type1-6-1][2]*eyb[2])
...
def MakeLocalCoordsSystems(X1,Y1,Z1,X2,Y2,Z2):
    equal_diff=1.0e-6
    align=0
    #ez=[X2-X1,Y2-Y1,Z2-Z1]
    ez0=X2-X1
    ez1=Y2-Y1
    ez2=Z2-Z1
    #norm=math.sqrt(ez[0]**2+ez[1]**2+ez[2]**2)
    norm=math.sqrt(ez0**2+ez1**2+ez2**2)
    #ez[0]=ez[0]/norm
    #ez[1]=ez[1]/norm
    #ez[2]=ez[2]/norm
    ez0/=norm
    ez1/=norm
    ez2/=norm
    #if abs(ez[1])<equal_diff and abs(ez[2])<equal_diff:
    if abs(ez1)<equal_diff and abs(ez2)<equal_diff:
        #ey=[0,0,1]
        ey0=0
        ey1=0
        ey2=1
        align=1
    #if abs(ez[0])<equal_diff and abs(ez[2])<equal_diff:
    if abs(ez0)<equal_diff and abs(ez2)<equal_diff:
        #ey=[1,0,0]
        ey0=1
        ey1=0
        ey2=0
        align=1
    #if abs(ez[0])<equal_diff and abs(ez[1])<equal_diff:
    if abs(ez0)<equal_diff and abs(ez1)<equal_diff:
        #ey=[0,1,0]
        ey0=0
        ey1=1
        ey2=0
...
#                 t[i]=-1.0*(ez[(i+1)%3+1-1]+ez[(i+1+1)%3+1-1])/ez[i]
        ...
        if maxpos!=2:
            ...
            t2=-1.0*(ez0+ez1)/ez2     
        #ey=t
        #norm=math.sqrt(ey[0]**2+ey[1]**2+ey[2]**2)
        norm=math.sqrt(t0**2+t1**2+t2**2)
        #ey[0]=ey[0]/norm
        #ey[1]=ey[1]/norm
        #ey[2]=ey[2]/norm
        ey0=t0/norm
